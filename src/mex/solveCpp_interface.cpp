/*
 * MEX interface for the solveCpp binary.
 *
...
#define LOG_FILENAME "solveCpp.log"

/*
 * MEX gateway function. The MEX file synopsis is as follows:
 *
 *      solveCpp P C r e [return] [algorithm]
 *
 * Parameters:
                  ...
                  int nrhs, const mxArray *prhs[])
{
    ...
    // Set algorithm
    dpp::CppPlanningAlgorithm alg;
    if (algorithmName.compare("boustrophedon") == 0) {
        alg = dpp::CppPlanningAlgorithm::BOUSTROPHEDON;
    }
    else {
    ...
    bool returnToInitial, dpp::CppPlanningAlgorithm algId) {
        */
    int result = solveCpp(polygon, VC, G, GA, r, e, Tour, Edges, Headings, cost, returnToInitial,
        alg);
